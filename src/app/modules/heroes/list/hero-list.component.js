System.register(["@angular/core", "@angular/router", "../service/hero.service"], function (exports_1, context_1) {
    "use strict";
    var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
        var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
        if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
        else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
        return c > 3 && r && Object.defineProperty(target, key, r), r;
    };
    var __metadata = (this && this.__metadata) || function (k, v) {
        if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
    };
    var __moduleName = context_1 && context_1.id;
    var core_1, router_1, hero_service_1, HeroListComponent;
    return {
        setters: [
            function (core_1_1) {
                core_1 = core_1_1;
            },
            function (router_1_1) {
                router_1 = router_1_1;
            },
            function (hero_service_1_1) {
                hero_service_1 = hero_service_1_1;
            }
        ],
        execute: function () {
            HeroListComponent = (function () {
                function HeroListComponent(heroService, router) {
                    this.heroService = heroService;
                    this.router = router;
                }
                HeroListComponent.prototype.ngOnInit = function () {
                    this.getHeroes();
                };
                HeroListComponent.prototype.getHeroes = function () {
                    var _this = this;
                    this.heroService.getHeroList().then(function (heroes) { return _this.heroes = heroes; });
                };
                HeroListComponent.prototype.onSelect = function (hero) {
                    this.selectedHero = hero;
                    this.goToDetail(this.selectedHero.id);
                };
                HeroListComponent.prototype.goToDetail = function (id) {
                    this.router.navigate(['heroDetail', id]);
                };
                HeroListComponent.prototype.addNewHero = function () {
                    this.router.navigate(['heroDetail', 0]);
                };
                return HeroListComponent;
            }());
            HeroListComponent = __decorate([
                core_1.Component({
                    moduleId: __moduleName,
                    selector: 'hero-list',
                    templateUrl: './hero-list.component.html',
                    styleUrls: ['./hero-list.component.css']
                }),
                __metadata("design:paramtypes", [hero_service_1.HeroService, router_1.Router])
            ], HeroListComponent);
            exports_1("HeroListComponent", HeroListComponent);
        }
    };
});
